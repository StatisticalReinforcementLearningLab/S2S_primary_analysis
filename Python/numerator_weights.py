...
# create dataframe that has columns: 
#  - id, time (1,...,)
...
# num_weight_stress = {}
# num_weight_no_stress = {}
# for id in participants_for_analysis:
#     if num_weight_stress_denom[id] == 0:
#         num_weight_stress[id] = np.nan
#     else: 
#         num_weight_stress[id] = round(num_weight_stress_num[id]/num_weight_stress_denom[id],2)
#     if num_weight_no_stress_denom[id] == 0:
#         num_weight_no_stress[id] = np.nan
#     else: 
#         num_weight_no_stress[id] = round(num_weight_no_stress_num[id]/num_weight_no_stress_denom[id],2)

import matplotlib.pyplot as plt
plt.hist(list(num_weight_stress.values()))
plt.show()

plt.hist(list(num_weight_no_stress.values()))
plt.show()

# Check number of available decision times stressed in user's lifetime: 
stress_ava_dp = {}
no_stress_ava_dp = {}
for id in participants_for_analysis:
    print("id: ", id)
    ...
    while day <= mrt_id_tenth_day:
        ...
        stress_avai_dec_points = available_dec_points[available_dec_points['isStress'] == True]
        no_stress_avai_dec_points = available_dec_points[available_dec_points['isStress'] == False]
        stress_ava_dp[id].append(stress_avai_dec_points.shape[0])
        no_stress_ava_dp[id].append(no_stress_avai_dec_points.shape[0])
        day = day + timedelta(days=1)

...
estimating_equation_dgm_sam <- function(
    ...
    for (i in 1:num_users)
    {
        ...
        Zdm[[i]] <- cbind(rep(1, num_dec_points), X[[i]])
        g_M_Zdm[[i]] <- Zdm[[i]]
        g_Y_Zdm[[i]] <- Ldm[[i]]
    }
    ...
    phat_x <- unlist(lapply(Xvec, function(i){p_x[i+1]}))
    init_weight_val <- ((phat_x / prob_A)^{Avec}) * (((1 - phat_x) / (1 - prob_A))^{1 - Avec}) 
    ...
    estimator_initial_value <- rep(0, length = 2 * dim_eta)
    solution_missingness <- tryCatch(
        ...
        error = function(cond)
        {
            ...
            message(cond)
            return(list(root = rep(NaN, 2 * dim_eta), msg = cond, f.root = rep(NaN, 2 * dim_eta)))
        }
    )
    ...
    for (i in 1:num_users)
    {
        ...
        for (t in 1:num_dec_points)
        {
            ...
            V21 <- V21 + tcrossprod(c(V21_const * frac_val * g_M_Zdm_id_t, 
                                    V21_const * (frac_val * (A_id_t - p_x[X_id[t] + 1]) - 1) * Zdm_id_t), X_id_t_arrow)

            V22 <- V22 + 120 * rbind(- I_id_t * W_id_t * exp_gMZXi_id_t * tcrossprod(g_M_Zdm_id_t), 
                                     - I_id_t * W_id_t * exp_gMZXi_id_t * (A_id_t - p_x[X_id[t] + 1]) * tcrossprod(Zdm_id_t, g_M_Zdm_id_t))
            
            V23 <- V23 + rbind(- I_id_t * W_id_t * exp_AZEta_id_t * sum(M_id_t) * A_id_t * tcrossprod(g_M_Zdm_id_t, Zdm_id_t), 
                               - I_id_t * W_id_t * exp_AZEta_id_t * sum(M_id_t) * (A_id_t - p_x[X_id[t] + 1]) * A_id_t * tcrossprod(Zdm_id_t , Zdm_id_t))
            ...
            V3_vec <- c(as.numeric(crossprod(V3_const, R1tm_val)) * g_Y_Zdm_id_t, 
                        as.numeric(crossprod(V3_const, R2tm_val)) * g_Y_Zdm_id_t, 
                        as.numeric(crossprod(V3_const, R1tm_val)) * (A_id_t - p_x[X_id[t] + 1]) * X_id_t_arrow, 
                        as.numeric(crossprod(V3_const, R2tm_val)) * (A_id_t - p_x[X_id[t] + 1]) * X_id_t_arrow)

            V32 <- V32 - tcrossprod(V3_vec, g_M_Zdm_id_t)
            V33 <- V33 - tcrossprod(V3_vec, A_id_t * Zdm_id_t)
            ...
            V34 <- V34 + (sum(V3_const) * 
                    rbind(cbind(- exp_gYZalpha1_id_t * tcrossprod(g_Y_Zdm_id_t), matrix(0, q, q)),
                          ...
                          cbind(- exp_gYZalpha1_id_t * tcrossprod((A_id_t - p_x[X_id[t] + 1]) * X_id_t_arrow, g_Y_Zdm_id_t), matrix(0, 2, q)),
                          cbind(matrix(0, 2, q), - exp_gYZalpha2_id_t * tcrossprod((A_id_t - p_x[X_id[t] + 1]) * X_id_t_arrow, g_Y_Zdm_id_t))))
            ...
            V35 <- V35 + rbind(cbind(expY1AtTerm * tcrossprod(g_Y_Zdm_id_t, X_id_t_arrow), 
                                     matrix(0, q, 2)), 
                               cbind(matrix(0, q, 2), 
                                     expY2AtTerm * tcrossprod(g_Y_Zdm_id_t, X_id_t_arrow)), 
    ...
    beta_se <- sqrt(diag(varcov)[11:14])
